...
public class PositionServiceTest {
  @InjectMocks PositionServiceImp positionServiceImp;
  private PositionDTO positionDTO;
  private Position position;
  private PositionFromApiDTO positionFromApiDTO;
  @Mock private PositionRepository positionRepository;
  @Mock private List<PositionFromApiDTO> positionsFromAPI;
  @Mock private ResultsDTO positions;
  ...
  public void checkPositionDetails_descriptionAndLocation_missing() {
    ...
    Exception exception =
        assertThrows(
            ...
            () -> positionServiceImp.checkPositionDetails(positionDTO));
  ...
  public void checkPositionDetails_description_tooLong() {
    positionDTO =
        new PositionDTO("pneumonoultramicroscopicsilicovolcanoconiosis123456789", "Budapest");
    Exception exception =
        assertThrows(
  ...
  public void checkPositionDetails_location_tooLong() {
    positionDTO =
        new PositionDTO(
            "Junior Java Developer", "pneumonoultramicroscopicsilicovolcanoconiosis123456789");
    Exception exception =
        assertThrows(
            InvalidInputException.class,
            () -> positionServiceImp.checkPositionDetails(positionDTO));
  ...
  public void findById_fail() {
    ...
    Exception exception =
        assertThrows(ResourceNotFoundException.class, () -> positionServiceImp.findById("1"));
  ...
  public void findAllByDescriptionAndLocation() {
    ...
    positionFromApiDTO =
        new PositionFromApiDTO(
            "Senior Software Engineer, React to React Native",
            "Flexible / Remote",
            "https://www.themuse.com/jobs/glossgenius/senior-software-engineer-react-to-react-native-4f4672");
    positionsFromAPI = new ArrayList<>(List.of(positionFromApiDTO));
    when(restTemplate.getForObject(
            "https://www.themuse.com/api/public/jobs?page=1&location=Flexible / Remote",
            ResultsDTO.class))
        .thenReturn(positions);
    when(positions.getListOfPositionsFromApi()).thenReturn(positionsFromAPI);
    when(positionRepository.findAllByDescriptionContainingAndLocation(
